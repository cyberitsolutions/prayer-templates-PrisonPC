...
TEMPLATES = abook_add abook_add_help abook_list abook_list_help \
  ...
  folders folders_help folders_toolbar footer \
  ...
  list_toolbar_bottom list_toolbar_top logout logout_raven \
...
TEMPLATES_FRONTEND = login login_hermes frontend_compose_timeout frontend_login_error \
  frontend_security frontend_session frontend_timeout container_start container_end \
  header footer
