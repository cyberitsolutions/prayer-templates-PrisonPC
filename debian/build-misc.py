...
with tempfile.TemporaryDirectory() as td:
    subprocess.check_call(
         ...
         *(['--include=devscripts,ca-certificates',  # install uscan
         ...
         'bookworm',
         '/dev/null',
         # We need the build products of build-prayer.py (i.e. prayer-templates-dev_*.deb).
         'debian/debian-12-PrisonPC-desktop.sources',
         # Due to the previous line, we also need to tell it where to find the regular sources.
         'debian/debian-12.sources',
         ])
    # debsign here?
    subprocess.check_call([
